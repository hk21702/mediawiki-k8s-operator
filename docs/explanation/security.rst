...
Secrets required to operate MediaWiki are generated by the MediaWiki charm using Python's secure ``secrets`` standard library. Generated secrets are a minimum of 64 bytes long, and are stored as a :ref:`Juju charm secret <juju:charm-secret>` for synchronization between units. The following fields in the MediaWiki charm configuration are generated by the charm:
...
The MediaWiki charm optionally allows for private SSH keys to be :ref:`configured <reference_configurations>` using a :ref:`Juju user secret <juju:user-secret>` for use in Git operations, such as when using an extension or skin hosted in a private repository. The SSH keys are injected into the relevant workload containers as needed, in a manner which ensures that they are inaccessible to the web server process.
